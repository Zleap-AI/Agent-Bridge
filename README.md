...
普通用户直接使用已经构建好的发布包，**不需要安装 Go，也不需要重新构建**。

1. 打开 [GitHub Releases](https://github.com/Zleap-AI/Agent-Bridge/releases/latest)。
2. 下载与当前操作系统及 CPU 架构匹配的发布包。
3. 解压发布包。

| 系统 | CPU | 发布包 |
| --- | --- | --- |
| Windows | x64 | `agent-bridge_v0.3.0_windows_amd64.zip` |
| Windows | ARM64 | `agent-bridge_v0.3.0_windows_arm64.zip` |
| macOS | Intel | `agent-bridge_v0.3.0_darwin_amd64.tar.gz` |
| macOS | Apple Silicon | `agent-bridge_v0.3.0_darwin_arm64.tar.gz` |
| Linux | x64 | `agent-bridge_v0.3.0_linux_amd64.tar.gz` |
| Linux | ARM64 | `agent-bridge_v0.3.0_linux_arm64.tar.gz` |
...
> `git clone` 得到的是项目源码，不是已构建的发布包。只有修改代码或手上没有匹配的发布包时，才需要按照[从源码构建](#从源码构建)安装 Go 并编译。
...
| 升级 | 下载新版本发布包并替换可执行文件；配置与会话记录会保留 |
...
只有开发、调试源码或没有匹配的发布包时才需要这一步。源码构建要求 Go `1.25` 或更高版本。

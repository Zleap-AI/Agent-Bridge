...
<p align="center">
  <img src="docs/assets/readme/architecture-overview-zh.png" alt="Agent-Bridge 架构" width="960" />
</p>
...
<p align="center">
  <img src="docs/assets/readme/local-console-zh.png" alt="Agent-Bridge Local Console" width="960" />
</p>

**Remote Console**：通过自托管 Server 管理远程 Device。

<p align="center">
  <img src="docs/assets/readme/remote-console-zh.png" alt="Agent-Bridge Remote Console" width="960" />
</p>
...
默认不需要输入公网 IP，脚本会自动识别并保存，也会为已启用的 UFW 或 firewalld 放行 TCP 9201。安装完成后，终端会显示一次性的 Setup URL。用浏览器打开该地址，设置 Owner Password 后即可进入 Remote Console。如果页面打不开，请确认云服务器安全组也已放行 TCP 9201。只有自动识别失败且链接中出现 `SERVER_PUBLIC_IP` 时，才需要将它替换为服务器的公网 IP。域名、HTTPS 和反向代理配置参见[开发者指南](#部署-server)。
...
使用非回环监听地址时，脚本会自动检测已启用的 UFW 或 firewalld，并放行对应的 TCP 端口；使用 `127.0.0.1` 时不会修改防火墙。云安全组以及自定义 iptables/nftables 规则仍需自行配置。如果脚本无法确认公网地址，输出链接中的 `SERVER_PUBLIC_IP` 需要手动替换；也可以重新安装时通过 `AGENT_BRIDGE_PUBLIC_URL` 显式指定公网地址。
...
<p align="center">
  <img src="docs/assets/readme/message-lifecycle-zh.png" alt="Message 经过 Agent-Bridge 的调用流程" width="960" />
</p>
